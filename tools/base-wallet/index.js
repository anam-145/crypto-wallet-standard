...
class BaseWallet {
  ...
  async getAddress() {
    ...
    return { address: wallet.address, chain: 'base' };
  }
  ...
  async getUSDCBalance(address) {
    return this.getTokenBalance(address, this.network.contracts.usdc);
  }

  async getOwnedTokens(address, maxTokens = 50) {
    try {
      const url = `https://api.etherscan.io/v2/api?chainid=84532&module=account&action=tokentx&address=${address}&startblock=0&endblock=99999999&sort=desc&apikey=${BASESCAN_API_KEY}`;
      ...
      for (const [tokenAddress, tokenInfo] of tokenMap) {
        try {
          ...
          console.warn(`[BaseWallet] Token balance check failed: ${tokenAddress}`, error.message);
        }
      }
